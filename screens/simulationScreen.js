...
function simulationScreenIcon(name){
  ...
  return `<svg class="simulation-v3-icon" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">${paths}</svg>`;
}
...
export function renderSimulationScreen({ services, context }) {
  ...
  return `<div class="screen screen--simulation screen-layout screen-layout--simulation simulation-v3">
    ...
    <form id="simulation-form" class="workspace simulation-v3-workspace" novalidate>
      ...
      <section class="condition-panel simulation-v3-condition" aria-labelledby="conditionTitle">
        <div class="panel-head"><div><small>CHANGE CONDITIONS</small><h2 id="conditionTitle">条件を変更</h2></div><span>元の記録を初期値に使用</span></div>
        <div class="condition-body">
          <p class="simulation-v3-condition__lead">変更した項目だけを使って再計算します。入力すると比較結果が自動で更新されます。</p>
          <div class="measure-grid">
            <label class="measure-field"><span>距離</span><div><input name="distanceKm" type="number" inputmode="decimal" min="0.01" max="100" step="0.01" value="${distance.toFixed(1)}" aria-label="距離"><b>km</b></div><small class="simulation-v3-adjust"><button type="button" data-simulation-adjust="distanceKm:-0.5">−0.5</button><button type="button" data-simulation-adjust="distanceKm:0.5">＋0.5</button></small></label>
            <label class="measure-field"><span>実際に走った時間</span><div><input name="durationMinutes" type="number" inputmode="decimal" min="0.01" max="600" step="0.1" value="${Math.round(duration)}" aria-label="実際に走った時間"><b>分</b></div><small class="simulation-v3-adjust"><button type="button" data-simulation-adjust="durationMinutes:-5">−5分</button><button type="button" data-simulation-adjust="durationMinutes:5">＋5分</button></small></label>
          </div>
          ...
          <button class="secondary-action simulation-v3-reset" type="reset">元の条件に戻す</button>
        </div>
      </section>
      <section class="preview-panel simulation-v3-preview" aria-labelledby="previewTitle">
        ...
        <div class="semantic-note simulation-v3-compare-note"><strong>比較の見方</strong><span>元の保存記録と変更後の再計算結果を、同じ12部位・同じ計算方法で比較します。</span></div>
        <div id="simulation-result" class="simulation-result simulation-result--v3" aria-live="polite"></div>
        <div class="rof-boundary simulation-v3-subjective-note"><span>本人の記録</span><p>走る前後の疲労感は条件変更から推定せず、本人が実際に記録した値だけを扱います。</p></div>
      </section>
    </form>
    <section class="simulation-v3-next" aria-labelledby="simulationNextTitle">
      <div class="simulation-v3-section-head"><div><small>比較を次へつなぐ</small><h2 id="simulationNextTitle">この比較から確かめること</h2></div><p>比較結果を確認した後の操作を選べます。</p></div>
      <div class="simulation-v3-next-grid">
