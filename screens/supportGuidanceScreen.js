...
function renderSelectedItems(guidance) {
  ...
  return `<section class="public-help-selected" aria-labelledby="public-help-selected-title"><div class="section-heading section-heading--compact"><p>選択した内容</p><h2 id="public-help-selected-title">公式案内と同じ種類の項目</h2></div><ul>${guidance.selectedItems.map((item) => `<li>${escapeHtml(item.label)}</li>`).join("")}</ul><p>RunLoadは、症状の現在性、強さ、原因、緊急性を判定していません。誤入力または現在は当てはまらない場合は、身体の記録を確認・修正できます。</p></section>`;
}
...
function renderPriorityPanel(recordId, guidance) {
  ...
  return `<section class="public-help-priority" aria-labelledby="public-help-priority-title"><div>${renderStatusLabel("公的な窓口を先に確認", "attention")}<h2 id="public-help-priority-title">現在も症状がある場合は、アプリ操作より公的な案内を優先してください</h2><p>選んだ項目の中に、厚生労働省・消防庁の救急案内でも119番を検討する例として扱われる種類の内容があります。RunLoadが緊急性を判定したという意味ではありません。</p></div><div class="public-help-priority__actions">${renderPhoneAction({ numberHref: "tel:119", displayNumber: "119番", label: "電話機能で119番を開く" })}${mhlw ? externalLink(mhlw, "119番の目安を確認") : ""}</div>${recordId ? `<a class="text-link" href="#/record-input?recordId=${encodeURIComponent(recordId)}&subflow=subjective">身体の記録を確認・修正する</a>` : ""}</section>`;
}
...
function renderAppActions(experience) {
  ...
  return `<section class="public-help-app-actions" aria-labelledby="public-help-app-actions-title"><div class="section-heading section-heading--compact"><p>RunLoad内でできること</p><h2 id="public-help-app-actions-title">公的な案内とは分けて記録を整理する</h2></div><div class="result-activation-hub__grid"><a class="result-activation-hub__item" href="#/consultation?recordId=${encodeURIComponent(recordId)}"><strong>相談用に整理する</strong><small>身体の記録と走行事実を分けたメモを作る</small><span aria-hidden="true">→</span></a><a class="result-activation-hub__item" href="#/record-input?recordId=${encodeURIComponent(recordId)}&subflow=subjective"><strong>身体の記録を確認する</strong><small>誤入力や現在は当てはまらない項目を修正する</small><span aria-hidden="true">→</span></a><a class="result-activation-hub__item" href="#/result?recordId=${encodeURIComponent(recordId)}"><strong>結果へ戻る</strong><small>数値表示と身体の記録を分けて見直す</small><span aria-hidden="true">→</span></a></div></section>`;
}
...
export function renderSupportGuidanceScreen({ services, context }) {
  ...
  return `<section class="screen screen--support-guidance" data-public-help-version="${escapeHtml(guidance.version)}">
    ${renderPageHeading({ eyebrow: "身体の記録と公的な案内", title: "公的な相談先を確認する", description: "RunLoadの数値表示ではなく、入力した内容をもとに公式の窓口を自分で確認するための画面です。" })}
