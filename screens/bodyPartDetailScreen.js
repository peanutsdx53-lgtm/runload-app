...
export function renderBodyPartDetailScreen({ services, context }) {
  ...
  if (!experience || experience?.regionalV2ResultRecord?.model_version !== PRIMARY_REGIONAL_V2_MODEL_VERSION || !row) {
    return `<div class="screen screen--body-part-detail prototype-parity prototype-parity--result"><div class="detail-topbar"><a href="#/result${recordId ? `?recordId=${encodeURIComponent(recordId)}` : ""}">‹ <span>今回の結果</span></a><strong>部位詳細</strong><span></span></div><main class="detail-main"><section class="detail-title"><p class="eyebrow">BODY REGION DETAIL</p><h1>${escapeHtml(name)}</h1><p>この部位の数値を表示できません。</p></section><p class="compact-boundary">不足する条件を0や100で補いません。</p><a class="detail-return" href="#/result${recordId ? `?recordId=${encodeURIComponent(recordId)}` : ""}">今回の結果へ戻る</a></main></div>`;
  }
  ...
  return `<div class="screen screen--body-part-detail prototype-parity prototype-parity--result"><div class="detail-topbar"><a href="#/result?recordId=${encodeURIComponent(recordId)}">‹ <span>今回の結果</span></a><strong>部位詳細</strong><span></span></div><main class="detail-main"><section class="detail-title"><p class="eyebrow">BODY REGION DETAIL</p><h1>${escapeHtml(name)}</h1><p>${escapeHtml(formatLocalDate(experience.record.date))}の保存結果</p></section><section class="detail-hero"><div class="detail-locator">${locatorSvg(regionId)}</div><div class="detail-value"><small>今回の目安</small><strong>${fmt(row.value, 1)}</strong><div class="detail-comparison"><span><small>前回</small><b>${previous ? fmt(previous.row.value, 1) : "—"}</b></span><i></i><span><small>前回差</small><b>${finite(delta) ? signed(delta, 1) : "—"}</b></span></div></div></section><section class="trend-card"><div class="trend-head"><div><small>同じ部位</small><h2>保存記録の推移</h2></div><span>${finite(delta) ? `前回差 ${signed(delta, 1)}` : "前回比較なし"}</span></div><svg class="trend-svg" viewBox="0 0 320 145" role="img" aria-label="保存記録の推移">${trendSvg(history)}</svg><div class="trend-dates">${history.map((item) => `<span>${escapeHtml(shortDate(item.experience.record.date))}</span>`).join("")}</div><p>同じ部位・同じ計算方法・同じ基準で比べられる保存記録を表示します。</p></section><p class="compact-boundary">この部位自身のReference-100基準100との比較です。別の部位との順位ではありません。</p><a class="detail-return" href="#/result?recordId=${encodeURIComponent(recordId)}">今回の結果へ戻る</a></main></div>`;
}
