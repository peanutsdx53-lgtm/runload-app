...
export function renderResultScreen({ services, context }) {
  ...
  return `<div class="screen screen--result prototype-parity prototype-parity--result"><section class="intro"><div class="intro-heading"><div><p class="eyebrow">RESULT</p><h1>今回の走り</h1></div><span>${escapeHtml(formatLocalDate(record.date))}</span></div>${renderRunSummary(record)}</section>${renderRegional(regionalV2ResultRecord, allExperiences, record)}${renderFatigue(services, record)}<section class="understanding-link-wrap" aria-label="結果を見たあとの確認"><a class="understanding-link" href="#/interpretation-room?recordId=${encodeURIComponent(record.id)}&origin=result"><span><small>結果を見たあとに</small><strong>今回の結果を理解する</strong><em>表示された内容を順番に整理して確認します。</em></span><i aria-hidden="true">›</i></a></section><section class="history-link-wrap"><a class="history-link" href="#/history?view=trends&metric=region&period=28&anchorDate=${encodeURIComponent(record.date)}&recordId=${encodeURIComponent(record.id)}"><span><small>この日の記録</small><strong>履歴で見る</strong></span><i>›</i></a></section></div>`;
}
