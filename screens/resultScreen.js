...
export function renderResultScreen({ services, context }) {
  ...
  return `<div class="screen screen--result prototype-parity prototype-parity--result"><section class="intro"><div class="intro-heading"><div><p class="eyebrow">RESULT</p><h1>今回の走り</h1></div><span>${escapeHtml(formatLocalDate(record.date))}</span></div>${renderRunSummary(record)}</section>${renderRegional(regionalV2ResultRecord, allExperiences, record)}${renderFatigue(services, record)}<section class="activation-link-wrap"><a class="activation-link" href="#/activation?recordId=${encodeURIComponent(record.id)}"><span><small>この結果を次に使う</small><strong>振り返る・共有する・次を考える</strong></span><i>›</i></a></section><section class="history-link-wrap"><a class="history-link" href="#/history?view=trends&metric=region&period=28&anchorDate=${encodeURIComponent(record.date)}&recordId=${encodeURIComponent(record.id)}"><span><small>この日の記録</small><strong>履歴で見る</strong></span><i>›</i></a></section></div>`;
}
