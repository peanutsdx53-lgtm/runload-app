...
function renderRegional(resultRecord, allExperiences, record) {
  ...
  return `<section class="regional-section" data-primary-regional-card><div class="section-heading"><span class="section-index">01</span><div><small>BODY REGION</small><h2>身体の部位から見る</h2></div></div><div class="regional-shell"><div class="regional-topline"><p>12部位の目安</p><span>部位ごとの表示</span></div><ul class="direction-legend" aria-label="身体図の色"><li><i class="legend-dot above"></i><span>その部位の基準より上</span></li><li><i class="legend-dot reference"></i><span>その部位の基準付近</span></li><li><i class="legend-dot below"></i><span>その部位の基準より下</span></li><li><i class="legend-dot unavailable"></i><span>表示なし</span></li></ul><div class="regional-layout"><div class="map-column"><div class="body-map" aria-label="前面・後面・足裏の12部位図">${bodyMap(resultRecord, infos)}</div><p class="map-note">色で12部位を確認。部位をタップすると詳細・推移を開きます。</p><button class="mobile-region-trigger" type="button" data-action="open-result-region-sheet"><span><small>12部位比較</small><strong>数値・前回からの変化を見る</strong></span><i>›</i></button></div><aside class="desktop-region-panel" aria-label="部位一覧"><div class="desktop-panel-heading"><div><small>12 REGIONS</small><strong>12部位比較</strong></div><span>数値・前回からの変化</span></div><div class="view-toggle" role="group" aria-label="表示する部位"><button class="active" type="button" data-result-view="focus">基準または前回より上 <b>(${focusCount})</b></button><button type="button" data-result-view="all">全12部位</button></div><div class="value-marker-legend" aria-label="横軸の記号"><span><svg viewBox="0 0 16 16" aria-hidden="true"><circle cx="8" cy="8" r="4"></circle></svg>今回値</span><span><svg viewBox="0 0 16 16" aria-hidden="true"><path d="M8 3 13 8 8 13 3 8Z"></path></svg>前回値</span></div><div class="region-list" data-result-region-list="focus">${regionList(resultRecord, infos, "focus")}</div><div class="region-list" data-result-region-list="all" hidden>${regionList(resultRecord, infos, "all")}</div></aside></div>${renderFacts(record)}<p class="compact-boundary">各部位では、その部位自身の基準を100として比べます。距離は別の走行事実で、部位の数値へ掛けません。別部位どうしを順位付けしません。</p><p class="visually-hidden">12部位の目安は、各部位自身の基準条件を100として比べます。研究上、この比較方法をReference-100と呼びます。走行距離は別の記録事実であり、距離そのものを数値へ掛けません。100は安全値・正常値・初心者平均ではありません。</p></div><div class="sheet-overlay" data-prototype-region-sheet hidden><section class="region-sheet" role="dialog" aria-modal="true" aria-labelledby="region-sheet-title"><div class="grip"></div><div class="sheet-head"><div><p class="eyebrow">BODY REGION</p><h2 id="region-sheet-title">12部位比較</h2></div><button type="button" data-action="close-result-region-sheet" aria-label="閉じる">×</button></div><div class="sheet-tabs" role="tablist"><button class="active" type="button" data-result-mobile-view="focus" aria-selected="true">基準または前回より上 <b>(${focusCount})</b></button><button type="button" data-result-mobile-view="all" aria-selected="false">全12部位</button></div><div class="region-list mobile-list" data-result-region-mobile-list="focus">${regionList(resultRecord, infos, "focus")}</div><div class="region-list mobile-list" data-result-region-mobile-list="all" hidden>${regionList(resultRecord, infos, "all")}</div></section></div></section>`;
}
...
export function renderResultScreen({ services, context }) {
  ...
  return `<div class="screen screen--result screen-layout screen-layout--result"><section class="intro"><div class="intro-heading"><div><p class="eyebrow">RESULT</p><h1>今回の走り</h1></div><span>${escapeHtml(formatLocalDate(record.date))}</span></div>${renderRunSummary(record)}</section>${renderRegional(regionalV2ResultRecord, allExperiences, record)}${renderFatigue(services, record)}<section class="understanding-link-wrap" aria-label="結果を見たあとの確認"><a class="understanding-link" href="#/interpretation-room?recordId=${encodeURIComponent(record.id)}&origin=result"><span><small>結果を見たあとに</small><strong>今回の結果を整理する</strong><em>表示された内容を、基準・過去・計算に使った情報と一緒に確認します。</em></span><i aria-hidden="true">›</i></a></section><section class="history-link-wrap"><a class="history-link" href="#/history?view=trends&metric=region&period=28&anchorDate=${encodeURIComponent(record.date)}&recordId=${encodeURIComponent(record.id)}"><span><small>この日の記録</small><strong>履歴で見る</strong></span><i>›</i></a></section></div>`;
}
