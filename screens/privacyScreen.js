import { buildPrivacyOverview, PRIVACY_OVERVIEW_VERSION } from "../core/runloadCore.js";
...
export function renderPrivacyScreen({ services }) {
  ...
  return `<section class="screen screen--privacy" data-privacy-overview-version="${escapeHtml(PRIVACY_OVERVIEW_VERSION)}">
    ...
    ${renderBackupSection()}
    ${renderDeleteSection()}
    <footer class="source-boundary privacy-footer"><p>この画面は、新しいアプリで実際に行う保存・通信の境界を説明するものです。旧版の端末内データは自動移行・自動削除せず、このアプリからは読み込みません。法的な同意取得や医療情報管理制度への適合を示すものではありません。</p></footer>
  </section>`;
}
