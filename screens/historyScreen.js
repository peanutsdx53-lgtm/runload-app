...
function renderTrendChart(items, regionName, workspace) {
  ...
  const fixedReference = `<line x1="10" y1="${geometry.referenceY}" x2="96" y2="${geometry.referenceY}" class="regional-history-chart__reference"></line><text x="95" y="${Math.max(6, geometry.referenceY - 1.5)}" text-anchor="end" class="regional-history-chart__reference-label">基準</text>`;
  const pointTitles = geometry.points.map((point) => `<a href="#/result?recordId=${encodeURIComponent(point.experience.record.id)}"><circle cx="${point.x}" cy="${point.y}" r="2.8" class="regional-history-chart__point"><title>${escapeHtml(`${formatLocalDate(point.experience.record.date)} ${valueLabel}${formatNumber(point.conditionIndexExact, 1)}、基準からの差${Number(point.conditionIndexExact) >= 100 ? "+" : ""}${formatNumber(Number(point.conditionIndexExact) - 100, 1)}`)}</title></circle></a>`).join("");
...
function prototypeChartSvg(items, workspace, desktop = false) {
...
export function renderHistoryScreen({services,context}) {
  ...
  if(!workspace)return `<div class="screen screen--history prototype-parity prototype-parity--history"><section class="page-head"><div><p class="eyebrow">HISTORY</p><h1>履歴</h1><p>過去の記録を探し、同じ意味で比べられる記録を比較します。</p></div></section><section class="history-view"><div class="empty-records empty-records--initial"><small>SAVED RECORDS</small><strong>保存した記録はまだありません</strong><p>走行または休養を保存すると、ここから記録を探して比較できます。</p><a href="#/record-input">記録を始める</a></div></section></div>`;
  const compare=workspace.view==="trends";
  return `<div class="screen screen--history prototype-parity prototype-parity--history"><section class="page-head"><div><p class="eyebrow">HISTORY</p><h1>履歴</h1><p>過去の記録を探し、同じ意味で比べられる記録を比較します。</p></div></section><p class="visually-hidden">各部位では、その部位自身の基準を100として比べます。比率はその基準に対する値です。研究上、この比較方法をReference-100と呼びます。走行距離は別の記録事実です。走行距離そのものを部位の数値へ掛けません。100は安全値・正常値・初心者平均ではありません。</p><nav class="history-mode" aria-label="履歴の表示">${prototypeNavButton("記録を探す","日付・走行内容から確認",buildHref({view:"records",period:workspace.period,anchorDate:workspace.endDate}),!compare)}${prototypeNavButton("部位を比較","同じ部位の変化を見る",buildHref({view:"trends",metric:"region",period:workspace.period,anchorDate:workspace.endDate,regionId:workspace.regionId,display:workspace.regionalDisplay}),compare)}</nav>${compare?prototypeCompareView(workspace):prototypeRecordView(workspace,context)}${services.workflows.history.loadUndoEntry()?'<div class="history-undo" role="status"><p>直前に削除した記録を元に戻せます。</p><button type="button" data-action="undo-history-delete">削除を元に戻す</button></div>':""}</div>`;
}
