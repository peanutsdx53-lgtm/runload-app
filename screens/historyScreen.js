...
function prototypeRegionPicker(workspace) {
  return `<div class="sheet-overlay" data-history-region-overlay hidden><section class="region-sheet" role="dialog" aria-modal="true" aria-labelledby="history-region-title"><div class="grip"></div><div class="sheet-head"><div><p class="eyebrow">BODY REGION</p><h2 id="history-region-title">表示する部位</h2></div><button type="button" data-action="close-history-region-picker" aria-label="閉じる">×</button></div><div class="region-options">${REGIONS.map((region)=>`<button type="button" class="region-option${region.id===workspace.regionId?" active":""}" data-history-region-id="${escapeHtml(region.id)}"><span class="locator">${prototypeLocatorSvg(region.id)}</span><span><strong>${escapeHtml(bodyRegionFormalName(region.id,region.name))}</strong><small>${escapeHtml(prototypeRegionViewLabel(region.id))}</small></span></button>`).join("")}</div></section></div>`;
}
...
function prototypeRecordView(workspace,context) {
  ...
  return `<section class="history-view"><section class="records-head"><div class="comparison-title"><span class="section-index">01</span><div><small>SAVED RECORDS</small><h2>保存記録を探す</h2></div></div><span class="record-total">${rows.length}件</span></section><form class="record-filters" id="history-record-filter-form"><input type="hidden" name="view" value="records"><input type="hidden" name="period" value="${workspace.period}"><input type="hidden" name="anchorDate" value="${escapeHtml(workspace.endDate)}"><label><span>記録内を検索</span><input name="query" type="search" value="${escapeHtml(context.parameters.get("query")||"")}" placeholder="日付、コース、メモ"></label><div class="type-toggle" role="group" aria-label="記録の種類">${[["all","すべて"],["run","走行"],["rest","休養"]].map(([value,label])=>`<button type="button" class="${activity===value?"active":""}" data-history-record-type="${value}">${label}</button>`).join("")}</div></form><div class="record-list">${rows.length?rows.map((item)=>{const r=item.experience.record;return`<article class="record-item"><div class="record-item-head"><time class="${weekendClass(r.date)}" datetime="${escapeHtml(r.date)}">${escapeHtml(formatLocalDate(r.date))}（${escapeHtml(weekdayLabel(r.date))}）</time><span class="record-kind${r.activityType==="rest"?" rest":""}">${r.activityType==="rest"?"休養":"走行"}</span></div><h3>${escapeHtml(formatActivitySummary(r))}</h3><p>${escapeHtml(r.course?.name||"コース名なし")}${r.memo?`・${escapeHtml(r.memo)}`:""}</p><div class="record-actions"><a href="#/result?recordId=${encodeURIComponent(r.id)}">結果を見る</a><button type="button" data-action="delete-history-record" data-record-id="${escapeHtml(r.id)}" data-record-label="${escapeHtml(`${formatLocalDate(r.date)}の記録`)}">削除</button></div></article>`;}).join(""):'<div class="empty-records">条件に合う記録はありません。</div>'}</div></section>`;
}
