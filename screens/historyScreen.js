...
export function renderHistoryScreen({ services, context }) {
  ...
  if (!workspace) {
    return `<section class="screen screen--history">${renderPageHeading({ eyebrow: "保存した記録", title: "履歴", description: "保存した記録と比べられる推移を見返します。" })}${renderEmptyState({ title: "保存済みの記録がありません", description: "最初の走行または休養を保存してください。", actionLabel: "記録を始める", actionScreen: "record-input" })}</section>`;
  }
  const focusedRegional = workspace.view === "trends" && workspace.metric === "region";
  if (focusedRegional) {
