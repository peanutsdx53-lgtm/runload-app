...
(() => {
  ...
  const caller=fromParam==='record'||fromParam==='plan'?fromParam:'simulation';
  const backToCourse=document.querySelector('#backToCourse');
  if(backToCourse)backToCourse.href='../course-flow-ui-v16/?from='+encodeURIComponent(caller);
  let source = null;
  ...
  els.useCandidate.addEventListener('click',()=>{
    ...
    const canBack=history.length>1&&document.referrer.startsWith(location.origin)&&document.referrer.includes('/prototype/course-flow-ui-v16/');
    if(canBack){history.back();return;}
    location.href='../course-flow-ui-v16/?from='+encodeURIComponent(caller);
  });
