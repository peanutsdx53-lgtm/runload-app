...
let compare=true;
const defaultState={distance:5,duration:32,preset:'flat',runningFormat:'RUN',surface:'ASPHALT',upShare:30,upGrade:3,downShare:30,downGrade:3,runDistance:4,runDuration:26};
...
function render(){
 ...
 $('#currentPaceSummary').textContent=`${pace}・公園周回`;
 ...
 const chips=[distance,pace,$('#runningFormat').value==='RUN_WALK'?'RUN_WALK':assumptionLabel(built.preset),$('#surface').selectedOptions[0].textContent];
...
render();
['#courseRoute','#courseRouteMain'].forEach(s=>$(s)?.addEventListener('click',()=>{const note=document.querySelector('.snapshot-note');if(note){note.textContent='コース選択・設定は、コース設定画面と合わせて次のprototypeで再設計します。';note.scrollIntoView({block:'nearest',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'});}}));
