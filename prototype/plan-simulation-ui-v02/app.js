...
let compare=false;
const defaultState={distance:5,paceMin:6,paceSec:30,preset:'flat',runningFormat:'RUN',surface:'ASPHALT',upShare:30,upGrade:3,downShare:30,downGrade:3,runDistance:4,runDuration:26};
...
function paceSeconds(){const m=Number($('#paceMin').value),s=Number($('#paceSec').value);return m*60+s}
function normalizePaceInputs(){let m=Math.max(0,Math.floor(Number($('#paceMin').value)||0)),s=Math.round(Number($('#paceSec').value)||0);if(s>=60){m+=Math.floor(s/60);s%=60}if(s<0)s=0;$('#paceMin').value=m;$('#paceSec').value=String(s).padStart(2,'0')}
function surfaceComponents(){const v=$('#surface').value;if(v==='UNKNOWN')return[];return[{category:v,sharePercent:100}]}
function buildInput(){
 normalizePaceInputs();
 const distance=Number($('#distance').value);const seconds=paceSeconds();const format=$('#runningFormat').value;
 if(!(distance>0&&seconds>0))return{error:'距離とペースを確認してください。'};
 const record={runningFormat:format,distanceKm:distance,durationMinutes:distance*seconds/60,surfaceComponents:surfaceComponents(),runSetting:'OUTDOOR_ROUTE'};
 if(format==='RUN_WALK'){
   const rd=Number($('#runDistance').value),rt=Number($('#runDuration').value);if(!(rd>0&&rt>0))return{error:'RUN_WALKでは、走った区間の距離と時間が必要です。'};
   record.runningDistanceKm=rd;record.runningDurationMinutes=rt;
 }
 const preset=$('#coursePreset').value;
 let u=0,d=0,gu=0,gd=0;
 if(preset==='rolling'){u=30;d=30;gu=3;gd=3}
 if(preset==='custom'){u=Number($('#upShare').value||0);d=Number($('#downShare').value||0);gu=Number($('#upGrade').value||0);gd=Number($('#downGrade').value||0)}
 if(u<0||d<0||u+d>100||gu<0||gd<0)return{error:'上り区間と下り区間の割合・勾配を確認してください。'};
 if(preset!=='unknown'){record.uphillSharePercent=u;record.downhillSharePercent=d;record.uphillGradePercent=gu;record.downhillGradePercent=gd}
 return{record,preset};
}
function renderBody(result){
 const html=['front','back','sole'].map(view=>{
   const paths=regionDefs.filter(r=>r[3]===view).map(r=>{const x=result?.regions?.[r[0]];const v=x?.value;const label=`${r[2]}：${finite(v)?Number(v).toFixed(1):'表示なし'}`;return `<path class="region-path" data-direction="${stateFor(v)}" d="${r[4]}"><title>${label}</title></path>`}).join('');
...
function render(){
 const built=buildInput();const warning=$('#inputWarning');const calc=$('#calcState');$('#customGrade').hidden=$('#coursePreset').value!=='custom';$('#runWalkFields').hidden=$('#runningFormat').value!=='RUN_WALK';
 ...
 const pace=`${$('#paceMin').value}:${String($('#paceSec').value).padStart(2,'0')} /km`;
 const distance=`${Number($('#distance').value).toFixed(1)} km`;
 const chips=[distance,$('#runningFormat').value==='RUN_WALK'?'RUN_WALK':pace,assumptionLabel(built.preset),$('#surface').selectedOptions[0].textContent];
 $('#assumptionChips').innerHTML=chips.map(x=>`<span>${x}</span>`).join('');
 renderBody(result);renderList(result);
}
function reset(){
 $('#distance').value=defaultState.distance;$('#paceMin').value=defaultState.paceMin;$('#paceSec').value=String(defaultState.paceSec).padStart(2,'0');$('#coursePreset').value=defaultState.preset;$('#runningFormat').value=defaultState.runningFormat;$('#surface').value=defaultState.surface;$('#upShare').value=defaultState.upShare;$('#upGrade').value=defaultState.upGrade;$('#downShare').value=defaultState.downShare;$('#downGrade').value=defaultState.downGrade;$('#runDistance').value=defaultState.runDistance;$('#runDuration').value=defaultState.runDuration;render();
}
$$('input,select').forEach(el=>el.addEventListener('input',render));
$('#coursePreset').addEventListener('change',render);$('#runningFormat').addEventListener('change',render);
$('#toggleCompare').addEventListener('click',()=>{compare=!compare;$('#toggleCompare').setAttribute('aria-pressed',String(compare));$('#toggleCompare').textContent=compare?'Reference-100表示':'前回差を表示';render()});
