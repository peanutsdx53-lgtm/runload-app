...
const selectedCourse={name:'公園周回',preset:'flat',surface:'ASPHALT',surfaceLabel:'舗装路',gradeLabel:'ほぼ平坦',upShare:0,downShare:0,upGrade:0,downGrade:0};
...
function surfaceComponents(){const v=selectedCourse.surface;if(v==='UNKNOWN')return[];return[{category:v,sharePercent:100}]}
function buildInput(){
 ...
 record.uphillSharePercent=selectedCourse.upShare;record.downhillSharePercent=selectedCourse.downShare;record.uphillGradePercent=selectedCourse.upGrade;record.downhillGradePercent=selectedCourse.downGrade;
...
function render(){
 ...
 const chips=[distance,pace,$('#runningFormat').value==='RUN_WALK'?'RUN_WALK':selectedCourse.gradeLabel,selectedCourse.surfaceLabel];
