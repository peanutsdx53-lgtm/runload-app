...
const rpeEnabled=document.querySelector('#rpeEnabled');
const rpe=document.querySelector('#rpe');
const rpeOutput=document.querySelector('#rpeOutput');
function rpeLabel(v){
  const n=Number(v);
  if(n===0)return '0・休息に近い';
  if(n<=2)return n+'・楽';
  if(n<=4)return n+'・やや楽';
  if(n<=6)return n+'・ややきつい';
  if(n<=8)return n+'・きつい';
  return n+'・とてもきつい';
}
function updateReflection(){
  document.querySelector('#rpeControl').hidden=!rpeEnabled.checked;
  rpeOutput.textContent=rpeLabel(rpe.value);
  const filled=[
    rpeEnabled.checked,
    bodyInput,
    personalInput,
...
rpeEnabled.addEventListener('change',updateReflection);
rpe.addEventListener('input',updateReflection);
