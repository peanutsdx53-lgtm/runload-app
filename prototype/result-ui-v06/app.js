...
function navigateDetail(id){location.hash=`detail=${encodeURIComponent(id)}`;}
...
function showDetail(id){const r=byId.get(id);if(!r)return;const sheet=document.querySelector('#regionSheetOverlay');if(sheet&&!sheet.hidden)sheet.hidden=true;document.querySelector('#detailName').textContent=r.name;document.querySelector('#detailValue').textContent=finite(r.value)?r.value:'—';document.querySelector('#detailDirection').textContent=directionText(r.value);const pd=previousDelta(r);document.querySelector('#detailPrev').textContent=finite(pd)?`前回差 ${signed(pd)}`:'前回比較なし';document.querySelector('#detailLocator').innerHTML=locatorSvg(r,true);renderTrend(r);document.querySelector('#detailScreen').hidden=false;document.body.style.overflow='hidden';}
function hideDetail(){document.querySelector('#detailScreen').hidden=true;document.body.style.overflow='';}
...
const overlay=document.querySelector('#regionSheetOverlay');document.querySelector('#openRegionSheet').addEventListener('click',()=>{overlay.hidden=false;document.body.style.overflow='hidden';});document.querySelector('#closeRegionSheet').addEventListener('click',()=>{overlay.hidden=true;document.body.style.overflow='';});overlay.addEventListener('click',e=>{if(e.target===overlay){overlay.hidden=true;document.body.style.overflow='';}});
document.querySelector('#detailBack').addEventListener('click',returnFromDetail);document.querySelector('#detailReturn').addEventListener('click',returnFromDetail);window.addEventListener('hashchange',syncRoute);syncRoute();