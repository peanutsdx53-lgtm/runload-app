...
export function resolveRecordInputReturnState(context = {}) {
  ...
  if (!journey) {
    return Object.freeze({ restore: true, focusSelector: "#record-optional-title", notice: "入力途中を復元しました。", source: "resume" });
  }
...
export function handleRecordInputRouteChange(previousScreen = "", nextScreen = "") {
  const related = new Set(["record-input", "course-library", "course-editor", "gpx-analysis"]);
  const auxiliary = new Set(["course-library", "course-editor", "gpx-analysis"]);
