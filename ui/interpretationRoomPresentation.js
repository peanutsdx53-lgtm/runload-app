...
function renderRest(output) {
  return `<div class="interpretation-room interpretation-room--rest" data-interpretation-room-state="rest">
    ...
    <section class="interpretation-room-boundary">
...
function renderLegacy(output) {
  return `<div class="interpretation-room interpretation-room--legacy" data-interpretation-room-state="legacy">
    <header class="interpretation-room-hero">
...
function renderOverviewMap(output) {
  ...
  const views = BODY_REGION_VIEWS.map((view) => {
    const paths = view.paths.map(([id, d]) => {
      ...
      return `<a href="${escapeHtml(regionHref(output, id))}" class="interpretation-room-region-link" aria-label="${escapeHtml(`${label}。この部位を見る`)}">
        <path class="interpretation-room-region-shape" data-reference-direction="${escapeHtml(state)}" d="${d}"><title>${escapeHtml(label)}</title></path>
      </a>`;
    }).join("");
    return `<figure class="interpretation-room-body-view"><figcaption>${escapeHtml(view.title)}</figcaption><svg viewBox="70 10 160 430" role="img" aria-label="${escapeHtml(`${view.title}の部位図`)}"><g class="interpretation-room-silhouette">${view.silhouette}</g><g>${paths}</g></svg></figure>`;
  }).join("");

  const choices = regions.map((region) => `<a href="${escapeHtml(regionHref(output, region.regionId))}" class="interpretation-room-region-choice">
  ...
  return `<section class="interpretation-room-overview" aria-labelledby="interpretation-room-overview-title">
    ...
    <p class="interpretation-room-boundary-line">部位どうしの数値を順位付けする図ではありません。</p>
  </section>`;
}
...
function renderReferenceComparison(region, step = 1) {
  ...
  const referenceLine = reference?.available
    ? `<div class="interpretation-room-comparison-row interpretation-room-comparison-row--reference"><span>この部位の基準</span><strong>100</strong><i aria-hidden="true">→</i><span>今回</span><strong>${escapeHtml(current)}</strong><small>差 ${escapeHtml(referenceDifference)}</small></div>`
    : "";

  let previousLine = '<p class="interpretation-room-no-history">同じ方法で比べられる過去記録はまだありません。今回の値を次回の比較点として使えます。</p>';
  if (previous?.available) {
    const smallDifference = previous.direction === "LESS_THAN_ONE_POINT";
    previousLine = `<div class="interpretation-room-comparison-row interpretation-room-comparison-row--previous"><span>${escapeHtml(previous.date ? formatLocalDate(previous.date) : "前回")}</span><strong>${escapeHtml(number(previous.previousValue))}</strong><i aria-hidden="true">→</i><span>今回</span><strong>${escapeHtml(current)}</strong><small>${smallDifference ? `差 ${escapeHtml(signed(previous.difference))}（1ポイント未満）` : `差 ${escapeHtml(signed(previous.difference))}`}</small></div>`;
  }

  return `<section class="interpretation-room-selected" aria-labelledby="interpretation-room-selected-title">
    <div class="interpretation-room-section-head"><span>${escapeHtml(String(step))}</span><div><small>選んだ部位を見る</small><h2 id="interpretation-room-selected-title">${escapeHtml(region.label)}</h2></div></div>
    <div class="interpretation-room-current-result"><span>${escapeHtml(referenceText(reference.direction || ""))}</span><strong>${escapeHtml(current)}</strong><small>基準100との差 ${escapeHtml(referenceDifference)}</small></div>
    <div class="interpretation-room-comparisons">${referenceLine}${previousLine}</div>
  </section>`;
}
...
function renderCalculationPath(region, step = 2) {
  ...
  if (path.resolutionStatus === "UNAVAILABLE") {
    return `<section class="interpretation-room-calculation" aria-labelledby="interpretation-room-calculation-title">
      <div class="interpretation-room-section-head"><span>${escapeHtml(String(step))}</span><div><small>表示の作られ方</small><h2 id="interpretation-room-calculation-title">今回確認できる計算情報</h2></div></div>
      <p class="interpretation-room-lead">この保存記録からは、現在の計算経路を正確に説明できません。推測で補いません。</p>
    </section>`;
  }
  ...
  const speedCard = speed ? `<div class="interpretation-room-flow-step"><small>${exposure.type === "RUNNING_PHASE" ? "走った区間から計算" : "入力から計算"}</small><strong>${escapeHtml(paceFromSpeed(speed.value) || "走る速さ")}</strong><span>走る速さ</span></div>` : "";

  let flow = "";
  if (path.activeRoute === "SECTION_COMPOSED") {
    flow = `<div class="interpretation-room-flow interpretation-room-flow--segments"><div class="interpretation-room-flow-step"><small>今回のコース</small><strong>${escapeHtml(String(exposure.segmentCount || ""))}区間</strong><span>区間ごとの条件を使用</span></div><i aria-hidden="true">↓</i><div class="interpretation-room-flow-step"><strong>区間ごとに計算</strong></div><i aria-hidden="true">↓</i><div class="interpretation-room-flow-step"><strong>距離に応じてまとめる</strong></div><i aria-hidden="true">↓</i><div class="interpretation-room-flow-step interpretation-room-flow-step--result"><span>${escapeHtml(region.label)}</span><strong>${escapeHtml(number(region.value))}</strong></div></div>
      <p class="interpretation-room-precision-note">複数区間をまとめた結果です。この画面では各区間の最終採用経路まで断定しません。</p>`;
  } else {
    flow = `<div class="interpretation-room-input-facts">${factCards}</div><div class="interpretation-room-flow">${speedCard}<i aria-hidden="true">↓</i><div class="interpretation-room-flow-step"><small>この部位の計算</small><strong>${escapeHtml(region.label)}</strong></div><i aria-hidden="true">↓</i><div class="interpretation-room-flow-step interpretation-room-flow-step--result"><span>今回</span><strong>${escapeHtml(number(region.value))}</strong></div></div>`;
  }

  const conditional = path.conditionalInputs || [];
  const conditionalHtml = conditional.length
    ? `<aside class="interpretation-room-conditional"><strong>関係する条件として記録されています</strong><div>${conditional.map((item) => `<span><b>${escapeHtml(inputLabel(item))}</b><small>${escapeHtml(inputValue(item, exposure))}</small></span>`).join("")}</div><p>保存結果だけでは、これらが最終値へ採用された経路をここで断定しません。</p></aside>`
    : "";

  const context = path.contextOnlyInputs || [];
  const contextHtml = context.length
    ? `<details class="interpretation-room-context-only"><summary>記録はあるが、この部位の現在の数値計算には使わない情報</summary><div>${context.map((item) => `<span><strong>${escapeHtml(inputLabel(item))}</strong><small>${escapeHtml(inputValue(item, exposure))}</small></span>`).join("")}</div></details>`
    : "";
...
function rofPoint(label, item) {
  ...
  return `<div class="interpretation-room-rof-reading"><small>${escapeHtml(label)}</small><strong>${escapeHtml(number(value, 0))}<em>/10</em></strong><span>${escapeHtml(rofMeaningText(item))}</span></div>`;
}
...
function renderSubjective(output, step = 3) {
  ...
  const comparison = pair
    ? `<p class="interpretation-room-rof-difference">走る前 ${escapeHtml(number(pre.value, 0))} → 走った後 ${escapeHtml(number(post.value, 0))}　差 ${escapeHtml(signed(context.difference.value, 0))}</p>`
  ...
  return `<section class="interpretation-room-subjective" aria-labelledby="interpretation-room-subjective-title">
    ...
    ${comparison}
    <p class="interpretation-room-boundary-line">これは本人が記録した疲れの感じ方です。部位ごとの基準100とは別の情報で、回復度・安全性・けがの危険性を判定する数値ではありません。</p>
  </section>`;
}
...
function renderAdvanced(output, region) {
  ...
  return `<details class="interpretation-room-advanced"><summary>計算方法と研究上の背景を詳しく見る</summary><div>
...
export function renderInterpretationRoom({ output } = {}) {
  if (!output?.state?.targetAvailable) {
    return '<div class="interpretation-room interpretation-room--empty" data-interpretation-room-state="empty"><header class="interpretation-room-hero"><p>結果を整理する</p><h1>対象の保存記録がありません</h1><p>保存した走行記録から、結果を順番に整理します。</p></header><a class="interpretation-room-action interpretation-room-action--primary" href="#/record-input"><span><strong>記録を始める</strong><small>新しい走行記録を入力します。</small></span><i aria-hidden="true">›</i></a></div>';
  }
