...
export const INPUT_PURPOSE_GUIDANCE = Object.freeze([
  ...
  Object.freeze({
    ...
    title: "自分の感覚を別に残す",
    description: "きつさ、身体の記録、メモは、走行記録や部位の目安とは分けて見返します。",
  }),
]);
...
const CONDITION_DEFINITIONS = Object.freeze({
  ...
  "surface-footwear": Object.freeze({
    title: "路面・シューズ・足のつき方",
    description: "路面の硬さや凹凸、シューズ、足のつき方によって、足裏の圧や足部・下肢の動きが変わることがあります。",
...
export function buildRegionalConditionExplanation(row = {}) {
  ...
  return Object.freeze({
    ...
    statement: exposureOnlyConditionUnsupported
      ? "今回の走行条件による数値への影響は計算していません。走行量を中心に振り返り、関連する一般的な説明を示しています。"
      : nonReferenceConditionEvents.length
        ? "ここでは、今回の記録と関連する一般的な説明を示しています。"
        : "今回の記録では、走行量を中心に振り返ります。関連する一般的な説明を示しています。",
  });
}
