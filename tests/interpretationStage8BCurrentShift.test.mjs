...
await test('STAGE8B-CURRENT-SHIFT-MAPS-TO-LOCATE-COMPARE',()=>{
...
await test('STAGE8B-CURRENT-SHIFT-EXPLAINS-ONE-REGIONAL-UNDERSTANDING',()=>{
  const html=renderInterpretationRoom({output:output(),view:'explain',mode:'visual',origin:'result'});
  assert.match(html,/比較可能な前回101から今回104へ、同じ部位内で表示位置が変わっています/);
  assert.match(html,/まず股関節部の位置を確認し、比較可能な前回から今回への違いだけを見ます/);
});

await test('STAGE8B-CURRENT-SHIFT-DOES-NOT-MIX-ROF-LANE',()=>{
  const html=renderInterpretationRoom({output:output(),view:'explain',mode:'visual',origin:'result'});
...
console.log(JSON.stringify({suite:'Interpretation Stage 8B Current Shift Mapping',total:results.length,passed:results.length-failed.length,failed:failed.length,status:failed.length?'FAIL':'PASS',results},null,2));
