...
await test('VISUAL-MAPPING-MULTI-LAYER-USES-SEPARATE-LAYERS-PATTERN',()=>{
...
await test('VISUAL-MAPPING-MULTI-LAYER-KEEPS-DISTINCT-SCALES',()=>{
  const html=renderInterpretationRoom({output:output(),view:'explain',mode:'visual',origin:'result'});
...
await test('VISUAL-MAPPING-MULTI-LAYER-HAS-NO-INTER-LAYER-CAUSAL-ARROW',()=>{
  const html=renderInterpretationRoom({output:output(),view:'explain',mode:'visual',origin:'result'});
...
await test('VISUAL-MAPPING-MULTI-LAYER-STILL-FOCUSES-ONE-REGION',()=>{
  const html=renderInterpretationRoom({output:output(),view:'explain',mode:'visual',origin:'result'});
  assert.equal((html.match(/interpretation-body-region is-focus/g)||[]).length,1);
  assert.match(html,/前回 → 今回/);
});

await test('VISUAL-MAPPING-MULTI-LAYER-HAS-ONE-UNDERSTANDING-NOTE',()=>{
  const html=renderInterpretationRoom({output:output(),view:'explain',mode:'visual',origin:'result'});
...
console.log(JSON.stringify({suite:'Interpretation Multi Layer Visual Mapping',total:results.length,passed:results.length-failed.length,failed:failed.length,status:failed.length?'FAIL':'PASS',results},null,2));
